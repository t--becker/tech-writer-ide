...
import MarkdownToolBarView from './views/markdown-toolbar-view';
...
FileFunctions = require('./functions/file-functions.js');
MarkdownValidatorModel = require('./models/validator-model.js');
MarkdownFunctions = require('./functions/markdown-functions.js');
...
export default {
  ...
  markdownToolbarPanel: null,
  markdownToolbarpanelView: null,
...
*/
    ...
    //instantiate the Markdown functions
    markdownFunctions = new MarkdownFunctions();
...
*/
    ...
    this.markdownToolbarpanelView = new MarkdownToolBarView(state.MarkdownToolBarViewState);
    this.markdownToolbarPanel = atom.workspace.addTopPanel({
      item: this.markdownToolbarpanelView.getElement(),
      visible: false,
      className: 'markdown-toolbar'
    });
    //load the controller with the panels
    controller = new Controller(this.modalPanel, this.rightPanel, this.markdownPanel,
      this.markdownToolbarPanel);
...
*/
    ...
    //rightPanel visibility change callback:
    //from marvel comics api and populates the right panel with the data
    ...
    this.subscriptions.add(this.markdownPanel.onDidChangeVisible((visible) =>{
      if(visible){
        ...
        if(fileFunctions.isMarkdown()){
          ...
          let markdownModel = new MarkdownValidatorModel(markdownFunctions.validateMarkdown());
...
*/
  ...
  deactivate() {
    ...
    this.markdownValidatorView.destroy();
    this.tigerPackageView.destroy();
    this.markdownToolbarPanel.destroy();
  },
